...
docker exec keycloak bash -c "$kcadm create clients $kcadm_auth -r sonddr -s clientId=sonddr-frontend -s 'redirectUris=[\"http://$HOSTNAME/*\"]' -s 'webOrigins=[\"http://$HOSTNAME\"]' -s directAccessGrantsEnabled=true -s publicClient=true -s enabled=true"
